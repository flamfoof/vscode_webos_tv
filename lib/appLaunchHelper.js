...
function _execServer(cmd) {
        ...
        child.stdout.on('data', (data) => {
            ...
            if (data.includes('http://localhost')) {
                ...
                const finishIndex = data.indexOf('\n');
                var thisurl = data.slice(startIndex, finishIndex);
                return thisurl;
            } else if (data.includes('To debug your service')) {
                const startIndex = data.indexOf('localhost:') + 10;
                const finishIndex = data.indexOf('" on Node Inspector Client');
                const port = data.slice(startIndex, finishIndex);
                resolve(port);
            } else if (data.includes('Ares Hosted App is now running')) {
                resolve();
            }
        });
        let errString = '';
...
function launchChrome(url) {
    logger.log(`Checking this>>>...>>>>> ${appResult}`);
    spawn('cmd', ['/c', 'start', 'C:\\Users\\blabl\\Desktop\\work\\Chrome_v79\\chrome.exe', url]);
    // chromeLauncher
    //     .launch({
    //         startingUrl: url,
    //         chromeFlags: [
